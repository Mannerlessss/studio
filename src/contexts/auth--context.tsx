        ...
        try {
            ...
            await runTransaction(clientDb, async (transaction) => {
                ...
                for (const investmentDoc of activeInvestmentsSnapshot.docs) {
                    ...
                    // --- Corrected Date Difference Logic ---
                    const startOfDay = (d: Date) => new Date(d.getFullYear(), d.getMonth(), d.getDate());
                    const msInDay = 24 * 60 * 60 * 1000;
                    const daysSinceStart = Math.floor((startOfDay(now).getTime() - startOfDay(startDate).getTime()) / msInDay);
                    // --- End of Correction ---
                    
                    const totalDaysToProcess = Math.min(daysSinceStart, investment.durationDays);
